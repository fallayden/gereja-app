...
<x-card padding="p-0" class="overflow-hidden hover:shadow-lg transition group">
    <div class="flex flex-col sm:flex-row">
        <!-- Thumbnail -->
        <div class="w-full sm:w-52 md:w-64 lg:w-72 h-48 sm:h-auto sm:min-h-[200px] bg-slate-100 flex-shrink-0 relative overflow-hidden">
            @if($thumbnail)
                <img src="{{ asset('storage/' . $thumbnail) }}" alt="{{ $title }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-300">
            @else
                <div class="w-full h-full flex items-center justify-center text-slate-400 bg-slate-200 min-h-[180px]">
